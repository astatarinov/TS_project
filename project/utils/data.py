...
def create_calendar_features(df: pd.DataFrame):
    ...
    """
    assert isinstance(
        df.index, pd.core.indexes.datetimes.DatetimeIndex
...
def add_rolling_features(df: pd.DataFrame, column_name: str, days_num=30):
    ...
    """
    df["{}_mean_{}".format(column_name, days_num)] = (
        df[column_name].rolling(window=days_num).mean()
    )
    df["{}_std_{}".format(column_name, days_num)] = (
        df[column_name].rolling(window=days_num).std()
    )
    df["{}_min_{}".format(column_name, days_num)] = (
        df[column_name].rolling(window=days_num).min()
    )
    df["{}_max_{}".format(column_name, days_num)] = (
        df[column_name].rolling(window=days_num).max()
    )
    df["{}_median_{}".format(column_name, days_num)] = (
        df[column_name].rolling(window=days_num).median()
    )
...
def add_features_to_balances(
    ...
    custom_calendar = pd.read_csv(
        os.path.join(DATA_PATH, "custom_calendar.csv")
    ...
    res_df = (
        balances.merge(custom_calendar, left_index=True, right_index=True, how="left")
